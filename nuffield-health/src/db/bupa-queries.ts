...
const ADJUSTED_DENOMINATOR = 100 - MAX_BOOKING_POINTS; // 90
...
function computeNuffieldAdjusted(
  ...
  return ((rawScore - bookingEarned) / ADJUSTED_DENOMINATOR) * 100;
}

function computeBupaAdjusted(rawScore: number | null): number | null {
  if (rawScore == null) return null;
  return (rawScore / ADJUSTED_DENOMINATOR) * 100;
}

// Get the latest completed BUPA run
export async function getLatestBupaRun() {
  return db
    .select()
    .from(bupaScrapeRuns)
...
export async function getBupaRunHistory() {
  return db
    .select()
    .from(bupaScrapeRuns)
    .orderBy(desc(bupaScrapeRuns.started_at));
...
export async function getBupaMatchForConsultant(nuffieldSlug: string) {
  return db
    .select()
...
function buildDimensionComparisons(
  ...
  dims.push({
    ...
    bupa_value: bupa.plain_english_score,
    winner: compareNum(nuffield.plain_english_score, bupa.plain_english_score),
  });
...
export async function getConsultantComparison(
  ...
  const match = await getBupaMatchForConsultant(nuffieldSlug);
